...
※現在、本アプリは企業の方に公開しているため、動作の安全性を最優先とし、 **`main` ブランチへのマージは控えております。**
機能確認の際は、以下の**未マージブランチ**もぜひご覧ください。

| ブランチ名              | 機能概要                       | マージ状況 |
|------------------------|------------------------------|------------|
| `feat/jump-calendar`     | (2025/7/6)カレンダーの年ジャンプ機能追加 | 未マージ   |
| `feat/search-by-genre-all-time`  |  （2025/8/1）ジャンル別絞り込み機能追加 |  未マージ  |
...
プロファイルを分割していた際、`.gitignore`対象のファイルをステージングした状態で`.gitignore`を編集してしまったため、誤ってパスワードをリポジトリに公開してしまいました。
